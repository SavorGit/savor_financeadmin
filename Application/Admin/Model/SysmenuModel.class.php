...
class SysmenuModel extends Model{
    ...
    protected $tableName='finance_sysmenu';

    public function getList($where, $order='id desc', $start=0,$size=5){
        $data = array();
        $totalSql  = "SELECT COUNT(*) as num FROM `savor_finance_sysmenu` {$where} order by id desc";
        ...
        //进行分页数据查询 使用page类属性
        $getSql = "select * from `savor_finance_sysmenu` {$where} order by {$order} limit $start,$size";
    ...
    public function getInfo($id,$code='') {
        $data  = array();
        $getInfoSql  = "SELECT * FROM `savor_finance_sysmenu` WHERE id = '{$id}'";
    ...
    public function getAllList() {
        $data  = array();
        $getListSql  = "SELECT * FROM `savor_finance_sysmenu` WHERE isenable=1 order by displayorder desc";
