...
class RolePrivModel extends Model{
    ...
    protected $trueTableName='savor_finance_role_priv';
    ...
    public function getInfoByroleid($code) {
        if($code){
            $getInfoSql  = "SELECT `roleid`, `m`,`c`,`a`,`menulevel`,`nodeid` FROM `savor_finance_role_priv` WHERE roleid = '{$code}'";
    ...
    public function getAllList() {
        $data  = array();
        $getListSql  = "SELECT * FROM `savor_finance_role_priv`";
    ...
    public function delData($id) {
        $delSql = "DELETE FROM `savor_finance_role_priv` WHERE roleid = '{$id}'";
    ...
    public function getMenuList($where,$order){
        $sql ="select n.id,n.name,rp.m,rp.a,rp.c,n.menulevel,n.media_id,n.select_media_id from savor_finance_role_priv as rp
               left join savor_finance_nodemenu as n on rp.nodeid=n.id 
