...
class syslogModel extends Model{
    protected $tableName='finance_syslog';
    public function getList($where, $order, $start=0,$size=5){
        $data = array();
        $totalSql  = "SELECT COUNT(*) as num FROM `savor_finance_syslog` {$where} order by id desc";
        ...
        //进行分页数据查询 使用page类属性
        $getSql = "select * from `savor_finance_syslog` {$where} order by {$order} limit $start,$size";
    ...
    public function getInfo($id) {
        $data  = array();
        $getInfoSql  = "SELECT * FROM `savor_finance_syslog` WHERE id = '{$id}'";
    ...
    public function delData($id) {
        $delSql = "DELETE FROM `savor_finance_syslog` WHERE id = '{$id}'";
